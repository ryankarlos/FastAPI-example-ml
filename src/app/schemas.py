...
class GetClientAge(BaseModel):
    ...
    def check_all_params_should_not_be_none(cls, values):
        gender, education, marriage = values.get("gender"), values.get("education"), values.get("marriage")
        if gender is None and education is None and marriage is None:
            raise ValueError("At least one of gender, education, marriage query strings must be passed with a value")
        return values
